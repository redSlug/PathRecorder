...
struct LivePathMapView: View {
    ...
    private func centerOnCurrentLocation() {
        guard isAutoCentering,
              ...
              Date().timeIntervalSince(lastCenterTime) > 0.5 else { return } // Throttle updates
        ...
        // Only update if region is significantly different or if region is nil (first time)
        ...
        ZStack {
            if let region = region {
                ...
                .onChange(of: locationManager.locations) { _ in
                    // Force update when locations array changes (important for new segments)
                    centerOnCurrentLocation()
                }
                .onChange(of: locationManager.isPaused) { isPaused in
                    // Re-enable auto-centering when recording resumes
                    if !isPaused && !isAutoCentering {
                        // Reset the throttling to allow immediate update on resume
                        lastCenterTime = Date().addingTimeInterval(-1)
                        isAutoCentering = true
                        // Force an immediate center update
                        if let currentLocation = locationManager.currentLocation {
                ...
                // Show loading state while waiting for first location
                VStack {
                    ProgressView()
                        .scaleEffect(1.5)
            ...
            VStack {
                HStack {
                    ...
                    .padding()
                    .opacity(region != nil ? 1 : 0.5) // Dim button when no location
                    .disabled(region == nil)
                }
                Spacer()
            }
